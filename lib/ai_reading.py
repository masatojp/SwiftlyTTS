...
class AIReadingClient:
    ...
    async def get_reading(self, text: str) -> str:
        ...
        system_prompt = (
            ...
            "{\n"
            "  \"original\": \"元のテキスト\",\n"
            "  \"yomi\": \"変換後のテキスト（基本的にひらがなと句読点のみ・単語はつなげる）\"\n"
            "}\n\n"
            "Rule:\n"
            "1. **基本的にひらがなと句読点**に変換して出力する。\n"
            "2. **単語と単語はスペースを入れずにつなげて**出力する。\n"
            "3. アルファベットや数字は、文脈に応じて自然な読み仮名（ひらがな）にする。\n"
            "4. 絵文字や記号は、読み上げに不要なら削除するか、意味を表す言葉に変換する。\n"
            "5. 文脈を考慮し、自然なアクセントやイントネーションになるような表記を目指す。\n"
            "6. 「｟」と「｠」で囲まれたテキストは、手動辞書による置換結果です。この部分は**絶対に**変更せず、囲まれたまま出力してください（例: input: ｟固定｠だよ -> output: ｟固定｠だよ）。"
        )
